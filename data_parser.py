...
def load_reaction_subject(feature, subject_id, normalizer, combined):
    '''
...
def load_data(task, paths, feature, emo_dim, normalize=True, win_len=200, hop_len=100, save=False,
              segment_train=True, combined = False, halve_val = False):
    '''
    ...
    data_file_name = f'data_{task}_{feature}_{emo_dim+"_" if len(emo_dim)>0 else ""}{"combined_" if combined else ""}{"halve" if halve_val else ""}_{"norm_" if normalize else ""}{win_len}_' \
    ...
    if halve_val:
        flag = np.ones(len(partition2subject['devel']))
        flag[:len(partition2subject['devel'])//2] = 0
        ...
        for subject_id in tqdm(subject_ids):
            if task == 'stress':
                ...
                features, labels, metas = load_reaction_subject(feature=feature, subject_id=subject_id,
                                                                normalizer=normalizer, combined=combined)
            if halve_val and partition == 'devel':
                if flag[subject_ids.index(subject_id)]:
                    data['train']['feature'].append(features)
                    data['train']['label'].append(labels)
                    data['train']['meta'].append(metas)
                else:
                    data['devel']['feature'].append(features)
                    data['devel']['label'].append(labels)
                    data['devel']['meta'].append(metas)
            else:
                if combined and partition == 'devel':
