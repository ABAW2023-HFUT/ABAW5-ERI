...
from model import TEMMA, TEMMA_Ensemble
...
def parse_args():
    ...
    parser.add_argument('--feature', nargs='+', default=['ResNet18', 'DeepSpectrum'], help="Specify the features used (only one).['ResNet18', 'VGGFace2', 'DeepSpectrum', 'eGeMAPs', 'FAUs']")
    ...
    parser.add_argument('--halve_val', default=False, action='store_true',
...
def main(args):
    ...
    for name in args.feature:
        data.append(load_data(args.task, args.paths, name, args.emo_dim, args.normalize,
                     args.win_len, args.hop_len, save=args.cache, combined=args.combined, halve_val=args.halve_val))
    data_loader = {}
    ...
    len_feature = data_loader['train'].dataset.get_feature_len_in_one_step()
    ...
    if args.eval_model is None:  # Train and validate for each seed
        ...
        for seed in seeds:
            ...
            # model1=torch.load('../model_103.pth')
            # model2=torch.load('../model_102.pth')

            # model = TEMMA_Ensemble(args,model1,model2)
            model = TEMMA(args, args.d_in, len_feature)
            ...
            if not args.predict:  # run evaluation only if test labels are available
                test_loss, test_score, _ = evaluate(args.task, model, data_loader['test'], loss_fn=loss_fn,
        ...
        # _, train_score, train_details = evaluate(args.task, model, data_loader['train'], loss_fn=loss_fn, eval_fn=eval_fn,
        #                              use_gpu=args.use_gpu)
        # print(f'Evaluating {model_file}:')
        # print(f'[Train {eval_str}]: {train_score:7.4f}')
        # for i in range(len(train_details)):
        #     print(f'[Train] | [{config.REACTION_LABELS[i]}]: {train_details[i]:>7.4f}')

        _, valid_score, valid_details = evaluate(args.task, model, data_loader['devel'], loss_fn=loss_fn, eval_fn=eval_fn,
                                  use_gpu=args.use_gpu)
        print(f'Evaluating {model_file}:')
        print(f'[Valid {eval_str}]: {valid_score:7.4f}')
        for i in range(len(valid_details)):
            print(f'[Valid] | [{config.REACTION_LABELS[i]}]: {valid_details[i]:>7.4f}')
        # if not args.predict:
        #     _, test_score, _ = evaluate(args.task, model, data_loader['test'], loss_fn=loss_fn, eval_fn=eval_fn,
        #                              use_gpu=args.use_gpu)
        #     print(f'[Test {eval_str}]: {test_score:7.4f}')
